# lavacone_trigger.py (refactored)
import math
import numpy as np
import time

class LavaConeTrigger:
    """
    ...
    def detect(self, lidar_obj, show_debug=False):
        ...
        in_band_2 = (
            (self.band_min < left_d  < 1.2) or
            (self.band_min < right_d < 1.2) or
            (self.band_min < front_d < 1.2)
        )
        ...
        if show_debug and now - self._last_dbg > self.DEBUG_INTERVAL:
            ...
            print(f"[LAVACONE_TRIGGER] L:{fmt(left_d)} R:{fmt(right_d)} "
        ...
        if not self.in_corridor and self.corridor_hold > self.enter_frames:
            ...
            print("🟠 [LAVACONE_TRIGGER] 복도 진입 감지 — 라바콘 모드 시작")
            return "ENTER"

        # EXIT 조건
        if self.in_corridor and self.corridor_release > self.exit_frames:
            self.in_corridor = False
            print("⚠️ [LAVACONE_TRIGGER] 복도 이탈 감지 — 모드 종료")
            return "EXIT"
